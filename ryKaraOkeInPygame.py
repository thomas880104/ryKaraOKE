...
class RySpectrogram:
    ...
    def Spectrum_thread(self):
        ...
        def show_spectrum(self, keyboard= None):
            ...
            nonlocal wav_i框_0, wav_i框_1, wav_i框_2
            ...
            #
            for xx in range(16):
                xxx= xx * self.width // 16
                c= 255-(xx*256*4//16)%256
                pg.draw.line(self.screen, (c,c,c),(xxx,0),(xxx,self.height) , 1)
            ...
            for n in range(-36,1):
                ...
                if n%12 in [3,5,7,8,10,0,2]: # C 大調，通通沒升記號。
                    ...
                    frequency_color = frequency_to_color((n/12)%1, is_color= True)                 
                    pg.draw.line(self.screen, frequency_color ,(0,y譜線),(self.width,y譜線) , 3)#彩色線
                    
                    noteName= noteNameL[n%len(noteNameL)] # noteNameL 在 ryPitch__ 裡面
                    noteMsg= '{}'.format(noteName)
                    noteText= aFont.render(noteMsg, True, frequency_color)#pg.Color('white'))
                    pg.draw.rect(self.screen, pg.Color('white'),[(x-20, y譜線-20),(20,20)])#在動的A-Ga-g文字底的白色框
                    self.screen.blit(noteText, (x-20, y譜線-20))#在動的A-Ga-g文字
            ...
            #評分        
            # 把 f0 放在這裡。
            f0= freq_from_autocorr(x0, self.Mic.sample_rate)#.frame_size) #self.Mic.sample_rate)
            f1= freq_from_autocorr(x1, self.Mic.sample_rate)#.frame_size) #self.Mic.sample_rate)
            ...
            if en0 > self.Mic.初框['mean']    + self.Mic.初框['std']*3:    
                x0聲音夠大= True
            if en1 > self.Mic.wav初框['mean'] + self.Mic.wav初框['std']*3: 
                x1聲音夠大= True
            
            if x0聲音夠大==False or f0 > 1000: #去掉雜音
                f0=1.0
            if x1聲音夠大==False or f1 > 1000: 
                f1=1.5 # 避開2倍 那會當作正確
            #s= abs(x).std()
            ...
            pitchScore_y= (self.height - pitchMatching) % self.height - 80# for y axis
            pitchScore_x= ((pitchMatching // self.height + 1) * 16) % self.width # for x axis
        ...
        while self.main_loop_is_running:
        ...
        if self.main_loop_is_running == False:
            pg.quit()
            self.Mic.stop()
            sys.exit()
        #
...
if __name__=='__main__':   
    ...
    if mainWin.allo ==True:
        if mainWin.NoNet ==True:
            mic= RyMic(musicfile = r'NoNetSongs/'+str(mainWin.filename)+r'.wav')
            mic.start()
